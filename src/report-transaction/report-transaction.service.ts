...
export class ReportTransactionService {
	...
	private async exportToExcel(data: GroupedReportItem[], groupBy: ReportGroupBy) {
		...
		worksheet.columns = [
			...
			{ header: 'Total Tax', key: 'totalTax', width: 18 },
		...
		const totalRow = {
			period: 'TOTAL',
			totalTransaction: data.reduce((acc, item) => acc + item.totalTransaction, 0),
			totalAmount: data.reduce((acc, item) => acc + item.totalAmount, 0),
			totalDiscount: data.reduce((acc, item) => acc + item.totalDiscount, 0),
			totalTax: data.reduce((acc, item) => acc + item.totalTax, 0),
			totalGrandTotal: data.reduce((acc, item) => acc + item.totalGrandTotal, 0),
		};
	...
	private async exportToPdf(data: GroupedReportItem[], groupBy: ReportGroupBy) {
		...
		doc.fontSize(10).text('Periode | Total Trx | Total Amount | Discount | Tax | Grand Total');
		doc.moveDown(0.5);

		data.forEach((item) => {
			doc.text(
				`${item.period} | ${item.totalTransaction} | ${item.totalAmount.toFixed(2)} | ${item.totalDiscount.toFixed(2)} | ${item.totalTax.toFixed(2)} | ${item.totalGrandTotal.toFixed(2)}`,
			);
		});
		...
		doc.moveDown();
		doc.text(
			`TOTAL | ${totals.totalTransaction} | ${totals.totalAmount.toFixed(2)} | ${totals.totalDiscount.toFixed(2)} | ${totals.totalTax.toFixed(2)} | ${totals.totalGrandTotal.toFixed(2)}`,
		);
